...
function onEdit(e) {
  ...
  if (lastRow > 1 && lastCol > 0) {
    ...
    console.log('Data check - Timestamp:', hasTimestamp, 'Name:', hasName, 'Email:', hasEmail);
...
/**
 * Test function to simulate adding a new row - FIXED VERSION
 */
function testAddNewRow() {
  ...
  // Immediately move it to the top (no setTimeout needed)
  console.log('Moving test row to top...');
  moveLastRowToTop(sheet);
...
function checkTriggers() {
  ...
  triggers.forEach(function(trigger) {
    ...
    console.log('  Source:', trigger.getTriggerSourceId());
    console.log('---');
  });
...
/**
 * Simple test to manually trigger the move function
 */
function manualMoveTest() {
  var sheet = SpreadsheetApp.getActiveSheet();
  console.log('Running manual move test...');
  moveLastRowToTop(sheet);
}
